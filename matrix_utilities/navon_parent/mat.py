...
def scalar_mul(M, alpha):
    ...
    return Mat(M.D,{(x,y):M[x,y]*alpha for x in M.D[0] for y in M.D[1]})
...
def transpose(M):
    ...
    return Mat((M.D[1],M.D[0]),{(y,x):M[x,y] for (x,y) in M.f})
...
def vector_matrix_mul(v, M):
    ...
    aVec = Vec(M.D[1],{})
    for (x,y) in M.f:
        thisCol = Vec(M.D[0],{m:M[m,y] for m in M.D[0]})
        aVec[y] = v*thisCol
...
def matrix_vector_mul(M, v):
    ...
    aVec = Vec(M.D[0],{})
    for (x,y) in M.f:
        thisRow = Vec(M.D[1],{m:M[x,m] for m in M.D[1]})
        aVec[x] = thisRow*v
...
def matrix_matrix_mul(A, B):
    ...
    assert A.D[1] == B.D[0]    
    return Mat((A.D[0],B.D[1]),{(x,y):Vec(A.D[1],{a:A[x,a] for a in A.D[1]}) * Vec(B.D[0],{b:B[b,y] for b in B.D[0]}) for (x,i) in A.f for (n,y) in B.f})
