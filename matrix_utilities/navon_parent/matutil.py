...
def dilate(M, center, scale_factor):
    "Given matrix M where each x,y is a vector, a center, and a scale_factor, return dilated matrix"
    #new_M = M.copy()
    return Mat(M.D,{(x,y):dilate(M[(x,y)], center, scale_factor) for (x,y) in M.F})
    #for (x,y) in M.F:
    #   new_M[(x,y)] = dilate(M[(x,y)], center, scale_factor)
    #return new_M